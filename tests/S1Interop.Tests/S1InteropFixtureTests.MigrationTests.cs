internal sealed partial class S1InteropFixtureTests
{
    ...
    private void SdkGenApplyGeneratesHoverboardFacadesFromReferenceMetadata()
    {
        ...
        try
        {
            ...
            WorkspaceAnalysis before = analyzer.Analyze(copiedProject);
            MigrationPlan plan = new MigrationPlanner().Plan(before);
            ProjectMigrationPlan projectPlan = plan.Projects.Single();
            Assert(
                projectPlan.Operations.Any(operation => operation.RuleId == "generate_sdk_facade"),
                "Hoverboard should plan SDK facade generation from its real source usage.");

            MigrationPlan sdkPlan = plan with
            {
                Projects =
                [
                    projectPlan with
                    {
                        Operations = projectPlan.Operations
                            .Where(operation => operation.RuleId == "generate_sdk_facade")
                            .ToArray()
                    }
                ]
            };
            MigrationApplyResult applyResult = new MigrationApplier().Apply(sdkPlan);
            Assert(
                applyResult.Operations.Any(operation => operation.RuleId == "generate_sdk_facade"),
                "SDK-only apply should generate Hoverboard facade declarations.");
            ...
            MigrationRollbackResult rollbackResult = new MigrationApplier().Rollback(applyResult.ManifestPath);
            Assert(rollbackResult.RemovedFiles.Contains(generatedFacade), "Rollback should remove the generated Hoverboard SDK facade.");
            Assert(!File.Exists(generatedFacade), "Rollback did not remove the generated Hoverboard SDK facade.");
            Assert(
                string.Equals(ComputeSha256(sourceProject), originalProjectHash, StringComparison.Ordinal),
