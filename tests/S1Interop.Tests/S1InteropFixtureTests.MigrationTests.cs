internal sealed partial class S1InteropFixtureTests
{
    ...
    private void MigrationUsesRuntimeGamePathSlotsForCustomRuntimeConfigurations()
    {
        ...
        try
        {
            ...
            Assert(
                ...
                !examplePropsText.Contains("RestoreAdditionalProjectSources", StringComparison.Ordinal),
                "local.build.props.example should show only runtime-specific game path slots.");
        }
        finally
        {
            DeleteDirectoryIfExists(tempRoot);
    ...
    private void SdkGenApplyGeneratesBarsGraphicsFacadesFromReferenceMetadata()
    {
        ...
        {
            Console.WriteLine("Skipping BarsGraphics SDK generation fixture because BarsGraphics or local game references are not available.");
            return;
        }
        ...
        try
        {
            string sourceCopy = Path.Combine(tempRoot, "BarsGraphicsSource");
            Directory.CreateDirectory(sourceCopy);
            CopyFixtureDirectory(sourceDirectory, sourceCopy);
            string copiedProject = Path.Combine(sourceCopy, "BarsGraphics.csproj");
            string generatedFacade = Path.Combine(sourceCopy, "S1Interop.Generated", "S1Interop.GlobalUsings.g.cs");

            ProcessResult dryRun = RunCli("sdkgen", copiedProject);
            Assert(
                dryRun.ExitCode == 0 &&
                dryRun.Output.Contains("install_s1interop_generator_package", StringComparison.Ordinal) &&
                dryRun.Output.Contains("generate_sdk_facade", StringComparison.Ordinal),
                $"BarsGraphics sdkgen dry-run should plan generator package installation and SDK facade generation from real source aliases. Output: {dryRun.Output}");

            ProcessResult apply = RunCli("sdkgen", copiedProject, "--apply");
            Assert(
                apply.ExitCode == 0 &&
                apply.Output.Contains("S1Interop migration applied", StringComparison.Ordinal) &&
                apply.Output.Contains("S1Interop.GlobalUsings.g.cs", StringComparison.Ordinal),
                $"BarsGraphics sdkgen --apply should generate facade declarations through the public CLI command. Output: {apply.Output}");
            Assert(File.Exists(generatedFacade), "BarsGraphics SDK facade should be generated by apply.");

            string projectText = File.ReadAllText(copiedProject);
            Assert(
                projectText.Contains(@"Include=""S1Interop.Generators""", StringComparison.Ordinal),
                "BarsGraphics sdkgen --apply should install the S1Interop generator package required by generated S1InteropType attributes.");
            ...
            BuildGeneratedSdkScaffoldAgainstBothReferenceSurfaces(
                tempRoot,
                "FreshBarsGraphicsGeneratedSdkMod",
                facadeSource,
                "BarsGraphicsProbe.cs",
                """
                namespace FreshBarsGraphicsGeneratedSdkMod;

                internal static class BarsGraphicsProbe
                {
            ...
            Assert(
                string.Equals(ComputeSha256(sourceProject), originalProjectHash, StringComparison.Ordinal),
                "BarsGraphics SDK generation fixture should not mutate the real project file.");
        }
        finally
        {
            DeleteDirectoryIfExists(tempRoot);
    ...
    private void SdkGenApplyGeneratesHoverboardFacadesFromReferenceMetadata()
    {
        string sourceDirectory = Path.Combine(WorkspaceRoot, "Hoverboard");
        string sourceProject = Path.Combine(sourceDirectory, "Hoverboard.csproj");
        string monoGamePath = @"D:\SteamLibrary\steamapps\common\Schedule I_alternate";
        ...
        try
        {
            ...
            Assert(
                ...
                dryRun.Output.Contains("install_s1interop_generator_package", StringComparison.Ordinal) &&
                dryRun.Output.Contains("generate_sdk_facade", StringComparison.Ordinal),
                $"Hoverboard sdkgen dry-run should plan generator package installation and SDK facade generation from real source usage. Output: {dryRun.Output}");
            ...
            Assert(
                ...
                apply.Output.Contains("S1Interop migration applied", StringComparison.Ordinal) &&
                apply.Output.Contains("S1Interop.GlobalUsings.g.cs", StringComparison.Ordinal),
                $"Hoverboard sdkgen --apply should generate facade declarations through the public CLI command. Output: {apply.Output}");
            Assert(File.Exists(generatedFacade), "Hoverboard SDK facade should be generated by apply.");

            string projectText = File.ReadAllText(copiedProject);
            Assert(
                projectText.Contains(@"Include=""S1Interop.Generators""", StringComparison.Ordinal),
                "Hoverboard sdkgen --apply should install the S1Interop generator package required by generated S1InteropType attributes.");
            string facadeSource = File.ReadAllText(generatedFacade);
            Assert(
                facadeSource.Contains("[assembly: S1Interop.S1InteropType(\"ScheduleOne.Skating.Skateboard\", Alias = \"Skateboard\", Il2CppTypeName = \"Il2CppScheduleOne.Skating.Skateboard\")]", StringComparison.Ordinal) &&
                facadeSource.Contains("[assembly: S1Interop.S1InteropType(\"ScheduleOne.Weather.WeatherConditions\", Alias = \"WeatherConditions\", Il2CppTypeName = \"Il2CppScheduleOne.Weather.WeatherConditions\")]", StringComparison.Ordinal),
                "Hoverboard SDK generation should infer concrete facade declarations from source usage and local reference metadata.");
            Assert(
                !facadeSource.Contains("global using Skateboard =", StringComparison.Ordinal) &&
            ...
            BuildGeneratedSdkScaffoldAgainstBothReferenceSurfaces(
                tempRoot,
                "FreshHoverboardGeneratedSdkMod",
                facadeSource,
                "HoverboardProbe.cs",
                """
    ...
    private void BuildGeneratedSdkScaffoldAgainstBothReferenceSurfaces(
        string tempRoot,
        string scaffoldName,
        string facadeSource,
        ...
        string il2CppGamePath)
    {
        ...
        File.WriteAllText(Path.Combine(scaffoldDirectory, probeFileName), probeSource);
        ...
        ProcessResult monoBuild = RunDotNet(
            ...
            $"-p:RestorePackagesPath={packageCache}",
            $"-p:RestoreAdditionalProjectSources={packageSource}");
        Assert(monoBuild.ExitCode == 0, $"Fresh scaffold with sdkgen output should build against Mono references. Output: {monoBuild.Output}");
        ...
        ProcessResult il2CppBuild = RunDotNet(
            ...
            $"-p:RestorePackagesPath={packageCache}",
            $"-p:RestoreAdditionalProjectSources={packageSource}");
        Assert(il2CppBuild.ExitCode == 0, $"Fresh scaffold with sdkgen output should build against IL2CPP references without source changes. Output: {il2CppBuild.Output}");
    }
