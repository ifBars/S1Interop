internal sealed partial class S1InteropFixtureTests
{
    ...
    public int RunQuick()
    {
        ...
        count++;
        S1InteropPatchTargetsValidateAgainstReferencedGameAssemblies();
        count++;
        OptionalS1InteropPatchTargetsCanUsePrivateRuntimeMethods();
        count++;
        S1InteropPatchTargetsWarnForAmbiguousOverloadsWithoutParameterTypes();
        count++;
        S1InteropPatchTargetsWarnForAccessorAndAggressiveInliningRisk();
        count++;
        ...
        count++;
        BackendNeutralSteamNetworkingHelpersSelectCompatiblePacketOverloads();
        count++;
        BackendNeutralSteamLobbyHelperResolvesCurrentLobbyAndMembers();
        count++;
        BackendNeutralMemberRegistryReportsRuntimeResolutionFailures();
        count++;
