...
S1Interop helps Schedule One mods share direct game access between Mono and IL2CPP without hand-maintaining every wrapper difference.

Use it for project analysis, SDK facade generation, rollbackable migrations, sandbox verification, and backend-neutral patch targets.

S1Interop is not a replacement for S1API, MAPI, SteamNetworkLib, or dedicated server APIs. Keep those libraries where they own the gameplay workflow. Use S1Interop for direct `ScheduleOne.*` / `Il2CppScheduleOne.*` access, type lookup, member binding, casts, delegates, and validation.
...
Most projects use both: the CLI writes declarations and migrates source; the generator turns those declarations into compiled helpers and diagnostics. If you only need the generated SDK surface, reference `S1Interop.Generators` and write declarations by hand.
