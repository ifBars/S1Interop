...
| `S1I002` | Error | A member override references an unknown owner alias — the `ownerAlias` does not match any declared `S1InteropType`. |
...
IL2CPP boundary diagnostics fire only when the compilation targets IL2CPP — detected by the presence of the `IL2CPP` preprocessor symbol or IL2CPP reference assemblies. They **never fire on Mono-only builds**.
