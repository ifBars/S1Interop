...
- Typed member accessors (properties, fields, methods) discovered from reference metadata or declared via `S1InteropMember`. These appear as named members on `Handle` and on the facade itself.
