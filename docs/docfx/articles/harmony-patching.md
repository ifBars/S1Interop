...
Patch target lookup uses `S1InteropMemberRegistry`. If a patch reports `SkippedMissingTarget`, inspect `S1Interop.Generated.S1InteropMemberRegistry.Reports` as well. It can tell you whether the active backend missed the owner type, the method, a declared parameter type, or an overload binding.
