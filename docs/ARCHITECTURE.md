...
The target API shape is type-first. `S1InteropNamespace` bulk-registers runtime types for broad SDK coverage without generating every member facade in the game. `S1InteropType` opts a specific game type into backend-neutral facades and discovers compatible public fields/properties plus unambiguous public methods from referenced Mono and IL2CPP metadata. The facade emitter uses concrete return and parameter types when the discovered metadata is backend-neutral today, such as scalar values, `string`, `object`, and `void` method returns. Game wrapper objects, collections, ambiguous overloads, generics, and by-ref shapes stay on the object/generic fallback helpers until conversion and handle-return rules are explicit. Discovery also skips generated backing-field patterns, including compiler/FishNet-style `BackingField` members, because mod code should usually use the real field or property surface. `S1InteropMember` is the override path for private members, readable aliases, ambiguous overloads, pinned method bindings, or specific migration-inferred reflection bindings; explicit member declarations do not currently carry enough type metadata to produce concrete typed signatures. Avoid designing new features that make developers manually enumerate every common public field/property or simple method after they already opted into the type.
