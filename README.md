...
The product direction is type-first SDK generation: adding or generating an `S1InteropType` now produces a backend-neutral facade with compatible public field/property helpers when reference metadata is available. `S1InteropMember` remains useful for private members, clearer aliases, ambiguous overloads, method bindings, or migration-inferred reflection patterns, but it should not be the normal way developers list every public field or property they want to read/write. See [`docs/PRODUCT_DIRECTION.md`](docs/PRODUCT_DIRECTION.md).
