...
`migrate --dual-runtime` can add IL2CPP configurations and rewrite source files, so run it as a dry-run first and apply it to a copied fixture or committed worktree. Mono-only projects with ordinary `Debug` / `Release` configurations are migrated by analyzer-inferred runtime, not just by configuration names containing `Mono`; generated `Il2cpp_Debug` / `Il2cpp_Release` configurations are also added to a sibling `.sln` when S1Interop can match the project there.
