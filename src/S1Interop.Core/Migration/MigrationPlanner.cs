            ...
            if (hasGeneratedMemberAccessTargets || hasAutomaticMemberAccessRewrites)
            {
                             ...
                             .OrderBy(path => path, StringComparer.OrdinalIgnoreCase))
                {
                    operations.Add(new MigrationOperation(
                        ...
                        "low",
                        true,
                        "Rewrite simple direct typeof(...).GetField(...), typeof(...).GetProperty(...), and AccessTools field/property/getter/setter lookups through generated S1InteropMemberRegistry FieldInfo/PropertyInfo accessors."));
                }
            }
